...
class NeuralModel():
    def __init__(self, config, name):
        ...
        # self.conv0_1 = conv_relu_batch(self.x, 19, 1,1, name="conv0_1")
        # # # conv1_1_d = tf.nn.dropout(self.conv1_1, self.keep_prob)
        # conv0_2 = conv_relu_batch(self.x, 19, 3,2, name="conv0_2")
        # # conv0_2_d = tf.nn.dropout(conv0_2, self.keep_prob)

        conv1_1 = conv_relu_batch(self.x, 64, 3,1, name="conv1_1")
        # conv1_1_d = tf.nn.dropout(self.conv1_1, self.keep_prob)
        conv1_2 = conv_relu_batch(conv1_1, 64, 3,2, name="conv1_2")
        # conv1_2_d = tf.nn.dropout(conv1_2, self.keep_prob)

        conv2_1 = conv_relu_batch(conv1_2, 128, 3,1, name="conv2_1")
        ...
        conv3_1 = conv_relu_batch(conv2_2, 256, 3,1, name="conv3_1")
        # conv3_1_d = tf.nn.dropout(conv3_1, self.keep_prob)
        conv3_2= conv_relu_batch(conv3_1, 256, 3,2, name="conv3_2")
        # conv3_2_d = tf.nn.dropout(conv3_2, self.keep_prob)
        conv3_3 = conv_relu_batch(conv3_2, 512, 3,1, name="conv3_3")
        ...
        #
        #
        fc7 = dense(self.fc6_d, 512, name="fc7")
